...
function OptionRow({
  ...
  return (
    ...
    >
      ...
      <input type="radio" checked={checked} onChange={onCorrect} className="h-5 w-5 accent-primary" />
...
export function MCQSingleEditor({ value, onChange }: EditorProps) {
  const data = value as MCQSingleData
  const updateOptions = (options: MCQSingleData['options']) => onChange({ ...data, options })
          ...
          <div className="space-y-2">
            {data.options.map((option) => (
              <OptionRow
                ...
                onCorrect={() =>
                  updateOptions(data.options.map((item) => ({ ...item, isCorrect: item.id === option.id })))
                }
