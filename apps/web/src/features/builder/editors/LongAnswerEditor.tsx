...
export function LongAnswerEditor({ value, onChange }: EditorProps) {
  const data = value as LongAnswerData
  return (
    <div className="space-y-3">
      <Input
        ...
        onChange={(event) =>
          onChange({ ...data, wordLimit: event.target.value ? Number(event.target.value) : undefined })
        }
      />
      <textarea
        value={data.rubric ?? ''}
